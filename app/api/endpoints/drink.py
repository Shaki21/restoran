...
async def create_drink(request: DrinkCreate, db: Session = Depends(get_db), current_user: DrinkDisplay = Depends(is_admin)):
...
async def update_drink(id: int, request: DrinkCreate, db: Session = Depends(get_db), current_user: DrinkDisplay = Depends(is_admin)):
    return DrinkController(db=db).update_drink(id=id, request=request)


@router.delete("/{id}", response_model=DrinkDelete)
async def delete_drink(id: int, db: Session = Depends(get_db), current_user: DrinkDisplay = Depends(is_admin)):
